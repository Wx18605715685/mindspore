...
class InternLMAttention(nn.Cell):
    ...
    def __init__(self,
        ...
        if self.use_kvcache_mgr:
            ...
            self.seq_length_tensor = Tensor([self.seq_length], dtype=mstype.int64)
            self.seq_length_tensor_pad = self.kvcache_concat_dim0((self.seq_length_tensor, self.kvcache_pad_tensor))
            self.seq_lengt_axis_tensor = Tensor([2], dtype=mstype.int64)
        ...
        if self.use_kvcache_mgr:
            ...
            if self.is_first_iteration:
                ...
                self.decoder_kvcache(value_past, value, valid_length, batch_index_pad, self.seq_lengt_axis_tensor_pad,
                                     self.seq_length_tensor_pad, self.seq_length_tensor_pad)
                if self.act_len:
                    slice_len = ops.shape(zactivate_len)[0]
                    # kv slice
                    key_present = self.slice(key_past, (0, 0, 0, 0), (bs, self.n_kv_head, slice_len, self.head_dim),
                                             (1, 1, 1, 1))
                    value_present = self.slice(value_past, (0, 0, 0, 0), (bs, self.n_kv_head, slice_len, self.head_dim),
                                               (1, 1, 1, 1))
                else:
                    key_present = self.slice(key_past, (0, 0, 0, 0),
                                             (bs, self.n_kv_head, self.seq_length, self.head_dim),
                                             (1, 1, 1, 1))
