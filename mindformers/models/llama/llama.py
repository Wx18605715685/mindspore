...
class LlamaModel(BaseModel):
    ...
    def construct(self, tokens: Tensor, input_position=None, batch_valid_length=None, batch_index=None,
        ...
        """
        # preprocess
        bs, seq_len = self.shape(tokens)  # 1, 310
        seq_range = self.range  # 1, 1, maxseq
        if self.is_first_iteration:
            cur_pos = batch_valid_length
            input_mask = self.cast(self.not_equal(tokens, self.pad_token_id), self.dtype)  # bs, seq
            mask = self.get_attention_mask(input_mask)
            if self.use_past:
                valid_length_vector = self.cast(self.less_past(seq_range, self.reshape(batch_valid_length, (-1, 1, 1))),
                                                self.dtype)  # bs, 1, maxseq
            else:
