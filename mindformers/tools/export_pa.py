...
from research.baichuan2.baichuan2_13b_pa import Baichuan13BV2ForCausalLM
...
# pylint: disable=W0612
def get_baichuan2_pa_inc_model_input(batch_size, seq_length, act_len, max_num_blocks_pre_batch, prefill):
    """get baichuan2 kv cache model input tuple."""
    if not prefill:
        seq_length = 1
    init_reset = not prefill
    ...
    if prefill:
        ...
        init_reset = ms.Tensor([init_reset], mstype.bool_)
        batch_valid_length = dummy_tensor(shape=[batch_size], dtype=ms.int32)
        batch_index = dummy_tensor(shape=[batch_size], dtype=ms.int32)
        block_tables = None
        slot_mapping = dummy_tensor(shape=[seq_length], dtype=ms.int32)
    else:
        input_ids = ms.Tensor(np.ones((batch_size, seq_length)), mstype.int32)
        input_position = ms.Tensor([127] * batch_size, mstype.int32)
        init_reset = ms.Tensor([init_reset], mstype.bool_)
        batch_valid_length = ms.Tensor([128] * batch_size, mstype.int32)
        batch_index = ms.Tensor([128] * batch_size, mstype.int32)
        block_tables = ms.Tensor(np.ones((batch_size * seq_length, max_num_blocks_pre_batch)), dtype=mstype.int32)
        slot_mapping = ms.Tensor(np.ones([batch_size * seq_length]), dtype=mstype.int32)
    ...
    print(f'batch_index shape: {batch_valid_length.shape}', flush=True)
    print(f'block_tables shape: {batch_valid_length.shape}', flush=True)
    print(f'slot_mapping shape: {batch_valid_length.shape}', flush=True)
    return input_ids, None, input_position, None, None, None, init_reset, \
        batch_valid_length, batch_index, block_tables, slot_mapping


def get_baichuan2_dyn_inc_model_input(batch_size, seq_length, act_len, prefill):
    ...
    print(f'batch_valid_length shape: {batch_valid_length.shape}', flush=True)
    if act_len:
        zactivate_len = dummy_tensor(shape=[None], dtype=ms.int32)
        print(f'zactivate_len shape: {zactivate_len.shape}', flush=True)
    else:
        zactivate_len = None
    return input_ids, None, input_position, None, None, None, init_reset, batch_valid_length, batch_index, zactivate_len
...
INCREMENT_MODEL_INPUT_MAP = {
    ...
    # "baichuan2": get_baichuan2_dyn_inc_model_input,
    "baichuan2": get_baichuan2_pa_inc_model_input,
...
def export_inc_model(config, batch_size, seq_length, model_type: str = 'MINDIR',
    ...
    suffix = model_type.lower()
    block_size = config.model.model_config.pa_block_size
    max_num_blocks_pre_batch = config.model.model_config.seq_length // block_size
    # export prefill
    filename = config.infer.prefill_model_path.rstrip("." + suffix)
    batch_size, seq_length = 1, None
    print(f'batch_size: {batch_size}', flush=True)
    print(f'seq_length: {seq_length}', flush=True)
    inputs = func(batch_size, seq_length, act_len, max_num_blocks_pre_batch, True)
    model.add_flags_recursive(is_first_iteration=True)
    ms.export(model, *inputs,
              file_name=filename,
              file_format=model_type.upper())
    ...
    batch_size, seq_length = config.model.model_config.batch_size, config.model.model_config.seq_length
    print(f'batch_size: {batch_size}', flush=True)
    print(f'seq_length: {seq_length}', flush=True)
    inputs = func(batch_size, seq_length, act_len, max_num_blocks_pre_batch, False)
...
def main(args_):
    ...
    act_len = config.model.model_config.use_act_len
