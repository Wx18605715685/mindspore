...
"""Contains utilities to handle headers to send in calls to Gitea."""
from typing import Dict, Optional, Union
import sys

from ._mds_folder import MdsFolder
from ..constant import MDS_HUB_DISABLE_IMPLICIT_TOKEN
...
def build_mds_headers(
    *,
    ...
    """
    Build headers dictionary to send in a Gitea call.
    ...
    token to the Hub, set `token=False` or set the `MDS_HUB_DISABLE_IMPLICIT_TOKEN`
    ...
    Args:
        ...
        token (`str`, `bool`, *optional*):
            The token to be sent in authorization header for the Hub call:
                ...
                - if a string, it is used as the Gitea token
                ...
                - if `None`, the token is read from the machine only except if
                  `MDS_HUB_DISABLE_IMPLICIT_TOKEN` env variable is set.
    ...
    if token is True:
        if cached_token is None:
            raise LocalTokenNotFoundError(
                ...
                " need to provide a token or be logged in to Gitea. See"
                " https://gitea.test.osinfra.cn/user/settings/applications."
            )
        return cached_token

    # Case implicit use of the token is forbidden by env variable
    if MDS_HUB_DISABLE_IMPLICIT_TOKEN:
        return None
    ...
    if is_write_action:
        if token is None:
            raise ValueError(
                ...
                " to provide a token or be logged in to Gitea with. See"
                " https://gitea.test.osinfra.cn/user/settings/applications."
            )
...
def _deduplicate_user_agent(user_agent: str) -> str:
    ...
    # Order is implicitly preserved by dictionary structure (see
