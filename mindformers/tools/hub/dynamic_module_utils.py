...
from mindformers.tools.hub.hub_utils import try_to_load_from_cache
...
def init_mf_modules():
    ...
    if MS_MODULES_CACHE in sys.path:
        return

    sys.path.append(MS_MODULES_CACHE)
    os.makedirs(MS_MODULES_CACHE, exist_ok=True)
    init_path = Path(MS_MODULES_CACHE) / "__init__.py"
...
def create_dynamic_module(name: Union[str, os.PathLike]):
    ...
    """
    init_mf_modules()
    dynamic_module_path = (Path(MS_MODULES_CACHE) / name).resolve()
...
def get_relative_import_files(module_file: Union[str, os.PathLike]) -> List[str]:
    ...
    while not no_change:
        ...
        no_change = len(new_import_files) == 0  # pylint: disable=C1801
...
def get_cached_module_file(
        ...
        _commit_hash: Optional[str] = None,  # pylint: disable=C0103
        **deprecated_kwargs,
) -> str:
    """
    ...
    submodule_path = Path(MS_MODULES_CACHE) / full_submodule
...
def custom_object_save(obj: Any, folder: Union[str, os.PathLike], config: Optional[Dict] = None) -> List[str]:
    ...
    def _set_auto_map_in_config(_config):  # pylint: disable=C0103
        ...
        if isinstance(_config, dict):
            ...
            auto_map[obj._auto_class] = full_name  # pylint: disable=W0212
            _config["auto_map"] = auto_map
        elif getattr(_config, "auto_map", None) is not None:
            _config.auto_map[obj._auto_class] = full_name    # pylint: disable=W0212
        else:
            _config.auto_map = {obj._auto_class: full_name}  # pylint: disable=W0212
