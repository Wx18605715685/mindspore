...
class PagedAttentionMgr(nn.Cell):
    ...
    def construct(self, key, value, slot_mapping):
        ...
        if self.input_layout == "BNSD":
            key = self.transpose(key, (0, 2, 1, 3))  # [b, n, s/1, d] -> [b, s/1, n, d]
            value = self.transpose(value, (0, 2, 1, 3))  # [b, n, s/1, d] -> [b, s/1, n, d]
    ...
    def paged_attn(self, query, batch_valid_length, block_tables):
        ...
        if self.input_layout == "BNSD":
            query = self.transpose(query, (0, 2, 1, 3))  # [b, n, s/1, d] -> [b, s/1, n, d]
